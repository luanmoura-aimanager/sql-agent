...
- **Pendente — Sessão D-3:** callback do `ChatAnthropic` (`BaseCallbackHandler.on_llm_end`) capturando `input_tokens`/`output_tokens` de `usage_metadata`; ContextVar acumulando por scope de request (mesmo padrão de [[project_sql_agent_2026_05_28]] do `request_id_var`); `pricing.json` versionado + loader + conversão pra USD; INSERT no `cost_events` no fim do request via integração com o middleware; log estruturado de `query_handled` ganha `input_tokens`, `output_tokens`, `cost_usd`.
