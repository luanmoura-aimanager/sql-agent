...
- **Governança — cost attribution fundação (Sessão D-1, 29/05/2026):**
  - **Reframe do escopo:** cost attribution virou trilho de 3-4 sessões (D-1 → D-4) ao decidir persistir custo em Postgres (categoria C — billing-grade, auditável) em vez de só logar. Razão: aprendizado real de stack de empresa (ponte pra DE/MLE) vence atalho. Path B escolhido sobre A (SQLite) e C (async + degradação).
  - **3 decisões travadas:** schema `cost_events` (BIGSERIAL PK, UUID `request_id`, `TIMESTAMPTZ`, `NUMERIC(12,8)`, `pricing_version`, 2 índices, 3 CHECKs, sem FK); SQLAlchemy 2.0 async **Core** + asyncpg + Alembic; pool (size=5, overflow=10, timeout=30s, recycle=1h, pre_ping=True). Razões nos comentários do `db.py`.
  - **Implementado em D-1:** `db.py` com `metadata` + `cost_events` Table + `engine` configurado + `init_db()`/`drop_db()` helpers; `docker-compose.yml` (Postgres 16-alpine + volume nomeado + healthcheck); `.env.example` com `DATABASE_URL`; deps no `requirements.txt`; `scripts/smoke_cost_events.py` (insert/select/CHECK de invariante); seção "Cost events DB" no README.
  - **Aprendizado-chave (não-óbvio):** SQLAlchemy `MetaData(naming_convention=...)` é mandatório quando usar Alembic — sem isso, Alembic gera nomes de constraint automáticos que mudam entre versões e geram diffs falsos só de renomeação em cada migration. Convenção explícita estabiliza nomes; Alembic gera só diffs reais.
- **Governança — Alembic + testcontainers (Sessão D-2, 30/05/2026):**
  - **Decisão única:** infra de teste contra Postgres real — `testcontainers-python` (não `pytest-postgresql` nem conectar ao docker-compose dele mesmo). Razões: reprodutibilidade (mesma imagem em dev/CI/máquina nova), isolamento (DB efêmero, dev não polui), padrão Nubank-tier (mesma sintaxe `with PostgresContainer(...) as pg:` em Java/Python/Go), CI-ready (Docker-in-Docker out-of-box no GitHub Actions etc.).
  - **Implementado:** `alembic init` + customizações em `alembic/env.py` (importa `metadata` de `db.py`, lê `DATABASE_URL` do env, troca `asyncpg`→`psycopg` pra Alembic só); primeira migration `0001_initial_cost_events.py` (hand-written matching autogenerate — `op.create_table` + 3 CHECKs + 2 índices + PK; comentário explica como validar pareando com autogenerate); removidos `init_db`/`drop_db` de `db.py` (Alembic é fonte única); `tests/conftest.py` com session-scoped fixture do testcontainers que sobe `postgres:16-alpine` efêmero e aplica `alembic upgrade head`; `tests/test_db_schema.py` com 5 testes (INSERT válido roundtrip, CHECK barra input_tokens negativo, CHECK barra cost_usd negativo, `server_default=NOW()` preenche occurred_at, naming convention aplicada); `pytest.ini` com `asyncio_mode=auto`.
  - **Decisões táticas:** (1) Alembic usa driver sync `psycopg` (migrations são one-shot, async adiciona complexidade sem ganho); (2) `docker_or_skip` fixture pula testes graciosamente se daemon ausente em vez de erro críptico; (3) fixture function-scoped trunca tabela em vez de drop/create — 10x mais rápido mantendo isolamento; (4) revision IDs com prefixo numérico (`0001_initial_cost_events`) em vez do hash random padrão pra navegação visual em diretório.
  - **Aprendizado-chave da sessão:** Alembic online mode espera driver **sync**. `asyncpg` é async-only — `env.py` precisa trocar prefixo do dialect (`postgresql+asyncpg://` → `postgresql+psycopg://`) só pra rodar migrations. O app continua usando `asyncpg` em runtime. Pragmatismo: migration é one-shot CLI, performance irrelevante; manter async ali quadruplica complexidade pra zero ganho.
- **Governança — Cost attribution callback + integração (Sessão D-3, 31/05/2026):**
  - **2 decisões travadas:** (1) `pricing.json` em formato nested (`{"version", "models": {model: {input_per_mtok, output_per_mtok}}}`) com per-MTok que bate com como Anthropic divulga preço; (2) aggregate via ContextVar durante o request + **1 INSERT por request** (não 1 por LLM call) — schema é "fact per request", N× menos linhas, side-mission de heterogeneity (router=haiku, react=sonnet) deferida.
  - **Decisão tática:** INSERT sync (await) no handler do `/query`. Async + degradação é Path C (Cap 4+).
  - **Implementado:** `pricing.json` (haiku 4.5 @ $0.80 in / $4.00 out per MTok); `pricing.py` (loader + validação + `calculate_cost_usd` com Decimal exato, quantize 8 casas pra bater NUMERIC(12,8)); `cost_callback.py` (`CostAccumulator` TypedDict, `cost_accumulator_var` ContextVar, `CostCallbackHandler.on_llm_end` lê `AIMessage.usage_metadata` com fallback defensivo pra `llm_output.token_usage`, singleton `cost_handler`); `agent.py` (`graph.invoke(config={"callbacks": [cost_handler]})` em **uma única linha** — langgraph propaga pra todos os nodes, sem instrumentar 3 sites); `main.py` (handler `/query` chama `init_accumulator(token_hash)`, depois do `run_agent` lê `read_accumulator()`, calcula `cost_usd`, INSERT no `cost_events` quando `call_count > 0 and model_name`, `reset_accumulator` no `finally`; log `query_handled` ganha `input_tokens`, `output_tokens`, `cost_usd`, `model_name`, `pricing_version`, `llm_call_count`).
  - **13 testes (`tests/test_cost_attribution.py`)**: 5 unitários de pricing (versão, cálculo exato, zero tokens, modelo desconhecido raises, quantize 8 casas), 4 unitários do callback (no-op fora de scope, acumula em múltiplas calls, missing usage gracioso, reset restaura default), 4 de integração com testcontainers + TestClient (INSERT happy path, sem LLM call → sem INSERT, log tem cost fields, isolamento entre requests).
  - **Aprendizado-chave (não-óbvio):** `graph.invoke(config={"callbacks": [...]})` propaga pra TODOS os nodes do langgraph automaticamente — instrumentar uma única linha no `run_agent` cobre router + sql_react_agent + tool loops do react. Se eu tivesse plugado em cada `model.invoke()` individualmente, teria triplo o código + risco de esquecer um site (ex: tool retry). **Pattern em qualquer integração langchain/langgraph: prefira o config no top-level invoke à instrumentação espalhada.**
- **Governança — Deployment (Sessão de fechamento Cap 3):**
  - **4 decisões travadas:** (1) Railway como plataforma (Postgres managed na mesma plataforma, CLI deploy, padrão fintech BR pós Heroku); (2) `alembic upgrade head` via `predeploy` no `railway.toml` — sem race entre réplicas, fail-closed (migration falhada = deploy rejeitado); (3) secrets via dashboard env vars com `API_TOKEN` fresh em prod (gerado via `secrets.token_urlsafe(32)`), `TRUSTED_PROXIES` setado pro IP do edge Railway pra XFF trust funcionar; (4) Dockerfile multi-stage `python:3.11-slim` (não alpine — wheels glibc) com non-root user + `--workers 1` (Railway escala horizontal, não vertical); GitHub Actions roda `pytest` em PRs/push pra main como gate antes do auto-deploy.
  - **Implementado:** `Dockerfile` (builder/runtime, venv isolada, non-root user `app`), `.dockerignore` (filtra .git, tests, py311env, .env), `railway.toml` (config-as-code com preDeployCommand + healthcheckPath=`/health` + restartPolicy ON_FAILURE), `.github/workflows/ci.yml` (Python 3.11 + pip cache + pytest com `API_TOKEN=ci-test-token` e `TESTCONTAINERS_RYUK_DISABLED=true`), README seção "Deploy on Railway" com sequência completa.
  - **Aprendizado-chave (não-óbvio):** **config-as-code do deploy é mandatório pra revisão atravessar deploy**. Sem `railway.toml`, mudar preDeployCommand vira "merge no dashboard sem PR" — perde rastreabilidade e dois desenvolvedores podem competir invisivelmente. Com arquivo versionado, PR review pega config de deploy junto com mudança de código.
- **Cap 3 governança FECHADO** depois desse PR. 8 blocos: auth → rate limit IP → rate limit token → XFF trust → structured logging → error detail leak → cost attribution (D-1 schema + D-2 migrations/testes + D-3 callback/integração) → deployment.
- **Próximo: consolidação não-código (entregável de fim de capítulo).**
  - Post no LinkedIn cobrindo a arquitetura do Cap 3 inteiro como case study.
  - Deck atualizado pra Emanuel cobrindo aprendizado consolidado.
  - **Depois, validação como usuário** contra o app deployado: 10-20 requests reais via curl/httpie, query no Postgres prod pra inspecionar `cost_events`, conferir que pipeline end-to-end funciona em ambiente real.
- **D-4 (queries CLI de relatório) ainda deferido como side-mission.** Útil quando aparecer demanda concreta de dashboard.
