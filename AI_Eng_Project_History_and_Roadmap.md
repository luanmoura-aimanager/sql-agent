...
- **Pendente — Sessão D-2 (próxima):** Alembic configurado (`alembic init`), primeira migration auto-gerada do schema do `db.py`, helpers de INSERT no padrão Core (`cost_events.insert().values(...)`), fixture de teste com **testcontainers** (sobe Postgres efêmero no pytest), testes do schema e dos CHECKs.
